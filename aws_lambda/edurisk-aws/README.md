...
```bash
edurisk-aws$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
edurisk-aws$ sam local start-api
edurisk-aws$ curl http://localhost:3000/classify_digit
```
...
```bash
edurisk-aws$ sam logs -n InferenceFunction --stack-name "edurisk-aws" --tail
```
...
```bash
sam delete --stack-name "edurisk-aws"
```
